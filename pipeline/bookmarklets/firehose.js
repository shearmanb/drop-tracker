/* Drop Pipeline - FIREHOSE bookmarklet  (geographic channel -> clipboard)
 * -----------------------------------------------------------------------
 * Click this while viewing a geographic drop channel. It reads every message
 * rendered on screen and COPIES them to your clipboard as JSON. Then you paste
 * that into the Worklist app's "Paste capture" box and click Save.
 *
 * Why copy instead of send? Discord's security policy (CSP) blocks its pages
 * from sending data to outside servers. Copying is not blocked, and the app
 * (on GitHub Pages) is what actually writes to the sheet. No Discord API, no
 * token, no network call from Discord at all.
 *
...
(function () {
  function channelName() {
    var sel = ['section[aria-label="Channel header"] h1',
      ...
      'header [class*="title"]', 'h1[class*="title"]'];
  ...
  try {
    ...
    var payload = JSON.stringify({ tab: 'raw_channel', rows: rows });
    copyText(payload, function (ok) {
      if (ok) toast('Firehose: copied ' + rows.length + ' messages from #' + chan + '. Now paste into the app’s "Paste capture" box.');
