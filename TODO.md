...
### ❓ Unknown / Unsure
- [ ] **BTL-AUDIT** — Are all bottle lists sourced from Google Sheets only, or is any master bottle list hardcoded in `index.html`? Audit and consolidate so the 69-bottle master lives exclusively in the Bottles sheet tab (once created) — no duplicate local copy.
...
- [ ] **DT-5** — Desktop store past drops: bottle names show old freeform strings instead of normalized bottle chips. Update past drops tab in desktop store panel to render tier-colored bottle chips the same way mobile does.
